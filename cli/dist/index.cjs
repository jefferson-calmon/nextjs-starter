...
${u}`),e.push({name:["env.ts"].join(""),dir:m.default.join(m.default.dirname(""),"src","config"),content:n}),e}function dt(o){return o.split(" ").map(Y).join(" ").split("/").map(Y).join("/")}var Q=require("@clack/prompts"),$=C(require("chalk"),1),$e=require("path");var Se=require("child_process");function y(o){return new Promise(e=>{(0,Se.exec)(o,(t,i,n)=>{e({error:t,stderr:n,stdout:i})})})}async function O(o){let e=(0,Q.spinner)(),[t,i,n]=[$.default.cyan(o.name),$.default.magenta(o.dir),(0,$e.join)(o.dir,o.name)];e.start(`Criando arquivo ${t} em ${i}`);let{success:r,error:s}=await xe(n,o.content.trim()),c=r?"foi criado com sucesso":"falhou ao ser criado";return e.stop(`O arquivo ${t} ${c} em ${i}`),{success:r,error:s,path:n}}async function ke({dependencies:o,dev:e=!1}){let t=(0,Q.spinner)(),i=$.default.magenta(e?"depend\xEAncias de dev":"depend\xEAncias"),n=o.join(" ");t.start(`Instalando ${i}: ${$.default.cyan(n)}`);let{error:r,stderr:s}=await y(`npm install ${e?"--save-dev":"--save"} ${n}`);return r?(t.stop(`Falha ao instalar ${i}`),console.error(s),{success:!1,error:r}):(t.stop(`As ${i} foram instaladas com sucesso`),{success:!0,dependencies:n})}var pt=w.z.tuple([w.z.enum(["model","context","env"]),w.z.string(),w.z.object({}),w.z.object({}).optional()]);async function Fe(...o){let[e,t]=pt.parse(o),i=await we(e,t),n=i[i.length-1];for(let r of i)await O(r).then(async s=>{r===n&&await y(`code ${s.path}`)})}var k=require("zod");var g=C(require("path"),1);var Ve=()=>`import { createFetchInstance } from 'codekit';
