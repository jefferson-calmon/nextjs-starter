...
`;function je(o){return{action:{api:mt,firebase:ft}[o]}}async function mt(){let o=await O({dir:g.default.join(g.default.resolve("src"),"services"),name:"api.ts",content:Ve()});await y(`code ${o.path}`)}async function ft(){await ke({dependencies:["firebase","firebase-admin","@next-firebase/data"]}),await O({dir:g.default.join(g.default.resolve("src"),"services","firebase"),name:"errors.ts",content:He()}),await O({dir:g.default.join(g.default.resolve("src"),"services","firebase"),name:"config.ts",content:Pe()}),await O({dir:g.default.join(g.default.resolve("src"),"services","firebase","client"),name:"index.ts",content:Ie()}),await O({dir:g.default.join(g.default.resolve("src"),"services","firebase","server"),name:"index.ts",content:Ne()}),await O({dir:g.default.join(g.default.resolve("src"),"services","firebase","server"),name:"serviceAccount.ts",content:Te()})}var _t=k.z.tuple([k.z.enum(["api","firebase"]),k.z.object({}),k.z.object({}).optional()]);async function qe(...o){let[e]=_t.parse(o),{action:t}=je(e);await t()}var I=new he;I.version(pe.version,"-v, --version","Exibir a vers\xE3o atual do app").name("app").description("CLI feita para automatizar tarefas repetitivas dentro do app");I.command("create <item> <name>").action(Fe);I.command("setup <item>").action(qe);I.parse(process.argv);
