...
`)}),this}_outputHelpIfRequested(e){let t=this._getHelpOption();t&&e.find(n=>t.is(n))&&(this.outputHelp(),this._exit(0,"commander.helpDisplayed","(outputHelp)"))}};function oe(o){return o.map(e=>{if(!e.startsWith("--inspect"))return e;let t,i="127.0.0.1",n="9229",r;return(r=e.match(/^(--inspect(-brk)?)$/))!==null?t=r[1]:(r=e.match(/^(--inspect(-brk|-port)?)=([^:]+)$/))!==null?(t=r[1],/^\d+$/.test(r[3])?n=r[3]:i=r[3]):(r=e.match(/^(--inspect(-brk|-port)?)=([^:]+):(\d+)$/))!==null&&(t=r[1],i=r[3],n=r[4]),t&&n!=="0"?`${t}=${i}:${parseInt(n)+1}`:e})}se.Command=G});var de=E(_=>{var{Argument:ce}=F(),{Command:X}=ae(),{CommanderError:rt,InvalidArgumentError:le}=S(),{Help:ot}=R(),{Option:ue}=U();_.program=new X;_.createCommand=o=>new X(o);_.createOption=(o,e)=>new ue(o,e);_.createArgument=(o,e)=>new ce(o,e);_.Command=X;_.Option=ue;_.Argument=ce;_.Help=ot;_.CommanderError=rt;_.InvalidArgumentError=le;_.InvalidOptionArgumentError=le});var he=C(de(),1),{program:wt,createCommand:St,createArgument:$t,createOption:kt,CommanderError:Ft,InvalidArgumentError:Vt,InvalidOptionArgumentError:It,Command:pe,Argument:Pt,Option:Ht,Help:Nt}=he.default;var me={name:"next-starter",version:"0.0.0",private:!0,scripts:{dev:"next dev --turbo",build:"next build",start:"next start",lint:"next lint","lint:style":"stylelint **/*.css --fix",app:"node cli/dist/index.cjs"},dependencies:{"@t3-oss/env-nextjs":"^0.11.1",codekit:"^2.11.0","framer-motion":"^11.11.10",geist:"^1.3.1","lucide-react":"^0.453.0",next:"^15.0.1","next-bricks":"^0.11.0","nextjs-toploader":"^3.7.15",polished:"^4.3.1",react:"^18.3.1","react-dom":"^18.3.1",zod:"^3.23.8"},devDependencies:{"@rocketseat/eslint-config":"^2.2.2","@types/node":"^20","@types/react":"^18","@types/react-dom":"^18",eslint:"^8","eslint-config-next":"14.2.5","eslint-config-prettier":"^9.1.0","eslint-plugin-prettier":"^5.2.1","eslint-plugin-simple-import-sort":"^12.1.1",postcss:"^8",stylelint:"^16.9.0","stylelint-config-clean-order":"^6.1.0","stylelint-config-standard-scss":"^13.1.0",typescript:"^5","typescript-plugin-css-modules":"^5.1.0"}};var w=require("zod");var ye=C(require("chalk"),1),m=C(require("path"),1);var fe=()=>`import { createEnv } from '@t3-oss/env-nextjs';
import { z } from 'zod';
...
`;var ge=o=>`/* eslint-disable @typescript-eslint/no-explicit-any */
...
`;var K="BaseModel.ts",_e=()=>`export interface BaseModel {
	id: string;
...
`,ve=o=>{let e=o.itemName.split("/").length-1,t=o.itemName.split("/").join("");return`import { Validations } from 'codekit';
...
`};var Oe=C(require("fs"),1),J=C(require("fs-extra"),1);function Ae(o){return Oe.default.existsSync(o)}async function Ce(o){return new Promise(e=>{J.default.readFile(o,{encoding:"utf-8"},(t,i)=>e(i))})}async function xe(o,e){return new Promise(t=>{J.default.outputFile(o,e,i=>{if(i)return t({success:!1,error:i});t({success:!0,error:null})})})}function Y(o){return o.charAt(0).toUpperCase()+o.slice(1)}var x=C(require("@clack/prompts"),1);function be(o){x.log.error(o),process.exit()}async function Ee(o){let e=await x.select(o);return x.isCancel(e)&&(x.cancel("Opera\xE7\xE3o cancelada"),process.exit()),e}async function we(o,e){return e=dt(e),await{model:ct,context:lt,env:ut}[o]({itemName:e})}function ct({itemName:o}){let e=[],t=m.default.join(m.default.dirname(""),"src","models",K);return Ae(t)||e.push({name:K,dir:m.default.join(m.default.dirname(""),"src","models"),content:_e()}),e.push({name:[o,".ts"].join(""),dir:m.default.join(m.default.dirname(""),"src","models"),content:ve({itemName:o})}),e}function lt({itemName:o}){let e=[];return e.push({name:[o,"Context",".tsx"].join(""),dir:m.default.join(m.default.dirname(""),"src","contexts"),content:ge({itemName:o})}),e}async function ut({itemName:o}){let e=[],t="env.ts",i=m.default.join(m.default.dirname(""),"src","config"),n=await Ce(m.default.join(i,t)),r=o.split(" ").join("_").toLocaleUpperCase(),s=await Ee({message:"Escolha o tipo de vari\xE1vel:",options:[{label:"Server",value:"server",hint:"Vari\xE1veis que ser\xE3o usadas apenas no ambiente do servidor"},{label:"Client",value:"client",hint:"Vari\xE1veis que ser\xE3o expostas no ambiente do cliente"}]});!r.startsWith("NEXT_PUBLIC")&&s==="client"&&(r="NEXT_PUBLIC_".concat(r)),r.startsWith("NEXT_PUBLIC")&&s==="server"&&(r=r.replace("NEXT_PUBLIC_","")),n||(n=fe()),n.includes(r)&&be(`Vari\xE1vel ${ye.default.cyan(r)} j\xE1 existe nas vari\xE1veis de ambiente`);let c=`${s}: {`,a=`		${r.toUpperCase()}: z.string(),
`;n=n.replace(c,`${c}
${a}`);let l="runtimeEnv: {",u=`		${r.toUpperCase()}: process.env.${r.toUpperCase()},
`;return n=n.replace(l,`${l}
${u}`),e.push({name:["env.ts"].join(""),dir:m.default.join(m.default.dirname(""),"src","config"),content:n}),e}function dt(o){return o.split(" ").map(Y).join(" ").split("/").map(Y).join("/")}var Q=require("@clack/prompts"),V=C(require("chalk"),1),$e=require("path");var Se=require("child_process");function y(o){return new Promise(e=>{(0,Se.exec)(o,(t,i,n)=>{e({error:t,stderr:n,stdout:i})})})}async function O(o){let e=(0,Q.spinner)(),[t,i,n]=[V.default.cyan(o.name),V.default.magenta(o.dir),(0,$e.join)(o.dir,o.name)];e.start(`Criando arquivo ${t} em ${i}`);let{success:r,error:s}=await xe(n,o.content.trim()),c=r?"foi criado com sucesso":"falhou ao ser criado";return e.stop(`O arquivo ${t} ${c} em ${i}`),{success:r,error:s,path:n}}async function ke({dependencies:o,dev:e=!1}){let t=(0,Q.spinner)(),i=e?"depend\xEAncias de dev":"depend\xEAncias",n=o.reduce((c,a)=>c+=`, ${V.default.cyan(a)}`,"");t.start(`Instalando ${i}: ${n}`);let{error:r,stderr:s}=await y(`npm install --quiet --no-progress --silent ${e?"--save-dev":"--save"} ${o.join(" ")}`);return r?(t.stop(`Falha ao instalar ${i}`),console.error(s),{success:!1,error:r}):(t.stop(`As ${i} foram instaladas com sucesso`),{success:!0})}var pt=w.z.tuple([w.z.enum(["model","context","env"]),w.z.string(),w.z.object({}),w.z.object({}).optional()]);async function Fe(...o){let[e,t]=pt.parse(o),i=await we(e,t),n=i[i.length-1];for(let r of i)await O(r).then(async s=>{r===n&&await y(`code ${s.path}`)})}var $=require("zod");var g=C(require("path"),1);var Ve=()=>`import { createFetchInstance } from 'codekit';
